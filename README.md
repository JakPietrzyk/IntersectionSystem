...
## My Main Focus
The main extension of the basic requirement in this task is the ability to implement any configuration of intersection. The backend correctly handles situations where there are multiple lanes on a given road. There can be several lanes in the same direction or in different directions.
...
Additionally Angular application can be started to launch interface in the browser.
...
Additionally, an interface was created to allow manual management of the intersection. This interface supports a single 
predefined intersection setup. Through the interface, users can control the traffic lights and manage the intersection manually.
...
- AllDirectionsRoadLine - allows to turn STRAIGHT, RIGHT or LEFT
...
- Implementing random vehicle generation on lanes and enabling the system to automatically execute subsequent steps without manual intervention.
- The frontend currently lacks automated tests, which could improve reliability and maintainability.